...
class HandDetector():
    def __init__(self,
        ...
        self.tip_ids = [4, 8, 12, 16, 20]    #mediapipe landmark indices for finger tips
    ...
    def find_landmark_positions(self, img):
        h, w, _ = img.shape
        self.lm_list_right = []
        self.lm_list_left = []
        ...
        if(self.results.multi_handedness):
            for hand_index, hand in enumerate(self.results.multi_handedness):
                ...
                for id, lm in enumerate(self.results.multi_hand_landmarks[hand_index].landmark):
    ...
    def fingers_extended(self, hand):
        fingers = []

        # Thumb: horizontal test, depends on type of hand (left or right)
        if hand == "Right":
            search_list = self.lm_list_right
            if search_list[self.tip_ids[0]][1] < search_list[self.tip_ids[0] - 2][1]:
                fingers.append(1)
            else:
                fingers.append(0)
        else:
            search_list = self.lm_list_left
            if search_list[self.tip_ds[0]][1] > search_list[self.tip_ids[0] - 2][1]:
                fingers.append(1)
            else:
                fingers.append(0)
        ...
        for id in range(1, 5):
            if search_list[self.tip_ids[id]][2] < search_list[self.tip_ids[id] - 2][2]:
                fingers.append(1)
            else:
                fingers.append(0)
